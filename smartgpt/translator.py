      ...
      "then": [
        {
          ...
          "type": "TextCompletion",
          "args": {
            "prompt_review": "the quality of the prompt is good, it matched all of the Criterias: [checked]other keys are referenced, [checked]requested AI to return a json response, [checked]the json response is stored in the key-value data, [checked]new key name end with seqnum", // must have 
...
def translate_to_instructions(task_info, model: str):
    try:
        user_prompt = (
            f"Our objective is to translate a task into a series of instructions. The task at hand is: {task_info['task']}\n"
