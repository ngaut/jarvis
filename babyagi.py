...
OPENAI_API_KEY = "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"

# Set variables
OBJECTIVE = "Write a very attractive blog to introduce tidb serverless, save the blog to tidbserverless.md."
...
def search(query, num_results=3):
    try:
        response = list(googlesearch.search(query, num_results=num_results))
        if response is None:
            return f"SearchOnlineAction RESULT: The online search for `{query}` appears to have failed."

        return response
    except HTTPError as http_err:
        if http_err.code == 429:
            time.sleep(30)
            return "search RESULT: Too many requests. Please try again later."
        else:
            return f"search RESULT: An HTTP error occurred: {http_err}"
...
def task_creation_agent(objective: str) -> List[Dict]:
    ...
    prompt = (
        ...
        f"Current tool options are [text-completion] {websearch_var} {user_input_var} {run_python_var}." # web-search is added automatically if SERPAPI exists
